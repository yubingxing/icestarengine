	...
	import com.ice.core.elements.Wall;
		...
		*/
		public class PlaneTesselation {
			...
			*/
			public static function tesselateFloor(f:fFloor,cubeSize:int):Array {
				...
				if(startCube!=endCube) {
					...
					xml1.@id+="_upSplit"
					if(xml1.@height>=(f.scene.gridSize >> 1)) toBeXTested[toBeXTested.length] = new fFloor(xml1,f.scene)

					var xml2:XML = f.xmlObj.copy()
					xml2.@y = splitY
					xml2.@id+="_downSplit"
					xml2.@height = f.y+f.depth-splitY
					if(xml2.@height>=(f.scene.gridSize >> 1)) toBeXTested[toBeXTested.length] = new fFloor(xml2,f.scene)
				...
				for(var i:int=0;i<l;i++) {
					...
					if(startCube!=endCube) {
						...
						xml1.@id+="_leftSplit"
						if(xml1.@width>=(f.scene.gridSize >> 1)) toBeReturned[toBeReturned.length] = new fFloor(xml1,f.scene)
        	
						xml2 = f.xmlObj.copy()
						xml2.@x = splitX
						xml2.@id+="_rightSplit"
						xml2.@width = f.x+f.width-splitX
						if(xml2.@width>=(f.scene.gridSize >> 1)) toBeReturned[toBeReturned.length] = new fFloor(xml2,f.scene)
			...
			* @return An array of floors
			*/
			public static function tesselateWall(w:Wall,cubeSize:int):Array {
				...
				if(w.vertical) {
					...
					if(startCube!=endCube) {
						...
						xml1.@id+="_upSplit"
						if(xml1.@size>=(w.scene.gridSize >> 1)) toBeZTested[toBeZTested.length] = new Wall(xml1,w.scene)
						
						var xml2:XML = w.xmlObj.copy()
						xml2.@y = splitY
						xml2.@id+="_downSplit"
						xml2.@size = w.y1-splitY
						if(xml2.@size>=(w.scene.gridSize >> 1)) toBeZTested[toBeZTested.length] = new Wall(xml2,w.scene)
				...
				for(var i:int=0;i<l;i++) {
					...
					if(startCube!=endCube) {
						...
						xml1.@id+="_lowerSplit"
						if(xml1.@height>=(w.scene.levelSize >> 1)) toBeReturned[toBeReturned.length] = new Wall(xml1,w.scene)
        	
						xml2 = w.xmlObj.copy()
						xml2.@z = splitZ
						xml2.@id+="_upperSplit"
						xml2.@height = w.top-splitZ
						if(xml2.@height>=(w.scene.levelSize >> 1)) toBeReturned[toBeReturned.length] = new Wall(xml2,w.scene)
