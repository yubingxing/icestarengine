package com.ice.core.renderEngines.flash9RenderEngine {
		...
		* @private
		*/
		public class Flash9PlaneRenderer extends Flash9ElementRenderer {
			...
			// Constructor
			function Flash9PlaneRenderer(rEngine:Flash9RenderEngine,element:fPlane,width:Number,height:Number,spriteToDraw:Sprite,spriteToShowHide:fElementContainer):void {
        	...
        	this.cacheTimer.removeEventListener(TimerEvent.TIMER, this.cacheTimerListener)
			  ...
			  for(var i:int=0;i<hl;i++) {
				...
				if(this.lightShadowsObj) {
					...
					for(i=0;i<il;i++) {
						Flash9RenderEngine.recursiveDelete(this.lightShadowsObj[i])
				...
				if(this.lightShadowsPl) {
					...
					for(i=0;i<il;i++) {
						Flash9RenderEngine.recursiveDelete(this.lightShadowsPl[i])
				...
				if(this.lightBumps) {
					...
					for(i=0;i<il;i++) {
						Flash9RenderEngine.recursiveDelete(this.lightBumps[i])
				...
				if(this.lightClips) {
					...
					for(i=0;i<il;i++) {
						...
						Flash9RenderEngine.recursiveDelete(this.lightClips[i])
				...
				for(j in this.occlusionSpots) {
					Flash9RenderEngine.recursiveDelete(this.occlusionSpots[j])
					delete this.occlusionSpots[j]
				}
				this.occlusionSpots = null

				Flash9RenderEngine.recursiveDelete(this.deformedSimpleShadowsLayer)
				Flash9RenderEngine.recursiveDelete(this.simpleShadowsLayer)
				Flash9RenderEngine.recursiveDelete(this.occlusionLayer)
				this.deformedSimpleShadowsLayer = null
				this.simpleShadowsLayer = null
				this.occlusionLayer = null

				// Return to object pool
				Flash9RenderEngine.recursiveDelete(this.baseContainer)
         ...
         if(autoStart) this.cacheTimer.start()
			...
			*/
			private function newMaterial(evt:fNewMaterialEvent):void {
				 ...
				 if(Flash9FloorRenderer.floorProjectionCache.fl==p) Flash9FloorRenderer.floorProjectionCache.fl = null
				 for(var i in Flash9FloorRenderer.wallProjectionCache) {
				 	if(int(i.substring(i.indexOf("_")+1))==p.uniqueId) Flash9FloorRenderer.wallProjectionCache=null
				 }
			...
			*/
			public override function renderShadow(light:fLight,other:fRenderableElement):void {
			   ...
			   if(other is fObject) {
			   	 ...
			   	 if(!(other.customData.flash9Renderer as Flash9ObjectRenderer).eraseShadows) msk = this.simpleShadowsLayer
			...
			*/
			public override function updateShadow(light:fLight,other:fRenderableElement):void {
			   ...
			   try {
			   	...
			   	var msk:Sprite
			   	if(other is fObject && !(other.customData.flash9Renderer as Flash9ObjectRenderer).eraseShadows) {
