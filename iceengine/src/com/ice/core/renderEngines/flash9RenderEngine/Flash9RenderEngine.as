package com.ice.core.renderEngines.flash9RenderEngine {
		...
		* @private
		*/
		public class Flash9RenderEngine implements fEngineRenderEngine {
		  	...
		  	private var viewHeight:Number = 0
				...
				* Class constructor
				*/
				public function Flash9RenderEngine(scene:fScene,container:Sprite):void {
				...
				*/
				public function initRenderFor(element:fRenderableElement):fElementContainer {
					...
					var renderer:Flash9ElementRenderer = this.createRendererFor(element)
				...
				*/
				public function stopRenderFor(element:fRenderableElement):void {
		  	 	...
		  	 	// Free graphics
		  	 	Flash9RenderEngine.recursiveDelete(element.container)
			  ...
			  **/
			  public function showElement(element:fRenderableElement):void {
			  	var r:Flash9ElementRenderer = element.customData.flash9Renderer
			  	if(!r.assetsCreated) {
			  		r.createAssets()
			  ...
			  private function applyPendingRenderMessages(element:fRenderableElement):void {
			  	...
			  	for(var i:int=0;i<l;i++) {
			  		var messageObj:RenderMessage = messages[i]
			  		var messageType:int = messageObj.message
			  		try {
			  			switch(messageType) {
								case AllRenderMessages.LIGHT_IN: r.lightIn(messageObj.target as fLight); break;
								case AllRenderMessages.LIGHT_OUT: r.lightOut(messageObj.target as fLight); break;
								case AllRenderMessages.LIGHT_RESET: r.lightReset(messageObj.target as fLight); break;
								case AllRenderMessages.RENDER_START: r.renderStart(messageObj.target as fLight); break;
								case AllRenderMessages.RENDER_LIGHT: r.renderLight(messageObj.target as fLight); break;
								case AllRenderMessages.RENDER_SHADOW: r.renderShadow(messageObj.target as fLight,messageObj.target2 as fRenderableElement); break;
								case AllRenderMessages.RENDER_FINISH: r.renderFinish(messageObj.target as fLight); break;
								case AllRenderMessages.UPDATE_SHADOW: r.updateShadow(messageObj.target as fLight,messageObj.target2 as fRenderableElement); break;
								case AllRenderMessages.REMOVE_SHADOW: r.removeShadow(messageObj.target as fLight,messageObj.target2 as fRenderableElement); break;
								case AllRenderMessages.GLOBAL_INTESITY_CHANGE: r.processGlobalIntensityChange(messageObj.target as fGlobalLight); break;
								case AllRenderMessages.GLOBAL_COLOR_CHANGE: r.processGlobalColorChange(messageObj.target as fGlobalLight); break;
								case AllRenderMessages.START_OCCLUSION: r.startOcclusion(messageObj.target as fCharacter); break;
								case AllRenderMessages.UPDATE_OCCLUSION: r.updateOcclusion(messageObj.target as fCharacter); break;
								case AllRenderMessages.STOP_OCCLUSION: r.stopOcclusion(messageObj.target as fCharacter); break;
			  			}
			  	  } catch(e:Error) {
			  ...
			  public function disableElement(element:fRenderableElement):void {
				...
				*/
				public function lightIn(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.LIGHT_IN,light)
				}
				...
				*/
				public function lightOut(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.LIGHT_OUT,light,null,true)
				...
				*/
				public function lightReset(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.LIGHT_RESET,light)
				}
				...
				*/
				public function renderStart(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.RENDER_START,light)
				}
				...
				*/
				public function renderLight(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.RENDER_LIGHT,light)
				}
				...
				*/
				public function renderShadow(element:fRenderableElement,light:fOmniLight,shadow:fRenderableElement):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.RENDER_SHADOW,light,shadow)
				}
				...
				*/
				public function renderFinish(element:fRenderableElement,light:fOmniLight):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.RENDER_FINISH,light)
				}
				...
				*/
				public function updateShadow(element:fRenderableElement,light:fOmniLight,shadow:fRenderableElement):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.UPDATE_SHADOW,light,shadow)
				}
				...
				*/
				public function removeShadow(element:fRenderableElement,light:fOmniLight,shadow:fRenderableElement):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.REMOVE_SHADOW,light,shadow,true)
				}
				...
				*/
				public function startOcclusion(element:fRenderableElement,character:fCharacter):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.START_OCCLUSION,character)
				}
				...
				*/
				public function updateOcclusion(element:fRenderableElement,character:fCharacter):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.UPDATE_OCCLUSION,character)
				}
				...
				*/
				public function stopOcclusion(element:fRenderableElement,character:fCharacter):void {
			  	...
			  	if(!r.screenVisible || fEngine.conserveMemory) r.renderMessages.addMessage(AllRenderMessages.STOP_OCCLUSION,character,null,true)
				}
					...
					for(var i in this.renderers) {
							...
							if(el!=null && found.indexOf(el)<0 && el.container.hitTestPoint(x,y,true)/*&& this.currentOccluding.indexOf(el)<0*/) {
									...
									if(el is fPlane) {
										...
										var r:Flash9PlaneRenderer = (el.customData.flash9Renderer as Flash9PlaneRenderer)
				...
				public function dispose():void {
					...
					for(var i in this.renderers) {
		  	 		...
		  	 		if(this.renderers[i].element) {
		  	 			Flash9RenderEngine.recursiveDelete(this.renderers[i].element.container)
				...
				*/
				private function processGlobalIntensityChange(evt:Event):void {
					for(var i in this.renderers) {
						...
						if(!this.renderers[i].screenVisible || fEngine.conserveMemory) this.renderers[i].renderMessages.addMessage(AllRenderMessages.GLOBAL_INTESITY_CHANGE,evt.target as fGlobalLight)
					}
				}
				...
				*/
				private function processGlobalColorChange(evt:Event):void {
					for(var i in this.renderers) {
						...
						if(!this.renderers[i].screenVisible || fEngine.conserveMemory) this.renderers[i].renderMessages.addMessage(AllRenderMessages.GLOBAL_COLOR_CHANGE,evt.target as fGlobalLight)
					}
				}
				...
				*/
				private function createRendererFor(element:fRenderableElement):Flash9ElementRenderer {
					...
					if(!this.renderers[element.uniqueId]) {
						...
						if(element is fFloor) element.customData.flash9Renderer = new Flash9FloorRenderer(this,spr,element as fFloor)
						else if(element is fWall) element.customData.flash9Renderer = new Flash9WallRenderer(this,spr,element as fWall)
						else if(element is fObject) element.customData.flash9Renderer = new Flash9ObjectRenderer(this,spr,element as fObject)
						else if(element is fBullet) element.customData.flash9Renderer = new Flash9BulletRenderer(this,spr,element as fBullet)
						else element.customData.flash9Renderer = new Flash9ElementRenderer(this,element,element.flashClip,spr)
				...
				public static function recursiveDelete(d:DisplayObjectContainer):void {
						...
						if(d.numChildren!=0) do {
							...
							if(c!=null) {
								...
								if(c is DisplayObjectContainer) Flash9RenderEngine.recursiveDelete(c as DisplayObjectContainer)
